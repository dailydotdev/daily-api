...
- **GraphORM mappings**: Only add entries in `src/graphorm/index.ts` when you need custom mapping/fields/transforms. For straightforward TypeORM entity reads (or when GraphORM relation detection is sufficient), avoid adding redundant GraphORM config.
