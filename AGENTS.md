...
- For **accept-only** GraphQL arguments (a client-reported value the resolver does not consume, e.g. an analytics-only `columns`), add the argument to the SDL only. Do not add a matching field to `FeedOptions`/arg types or GraphORM mappings — an unused type field is dead code. GraphQL still validates and accepts the arg, and clients that omit it (public REST routes, other feeds) keep working.
