...
import { Achievement, AchievementEventType } from '../src/entity/Achievement';
...
(async () => {
  ...
  // Load all achievements grouped by eventType
  ...
  log.info(
    {
      ...
      eventTypeCount: achievementsByEventType.size,
    },
  ...
  for (const [eventType, achievements] of achievementsByEventType) {
    ...
    if (!handler) {
      log.warn({ eventType }, 'No retroactive handler registered');
      continue;
    }
    ...
    try {
      ...
      log.error({ eventType, error }, 'Failed to process event type');
    }
  }
