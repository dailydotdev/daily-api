...
const engineeringReviewStatuses = new Set([
  FeedbackStatus.Processing,
  FeedbackStatus.Accepted,
]);
const closedFeedbackStatuses = new Set([
...
const worker: TypedWorker<'api.v1.feedback-updated'> = {
  ...
  handler: async ({ data }, con, logger): Promise<void> => {
    ...
    if (engineeringReviewStatuses.has(feedback.status)) {
    ...
    if (!closedFeedbackStatuses.has(feedback.status)) {
      return;
    }
