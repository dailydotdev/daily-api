...
const onOpportunityMatchChange = async (
  ...
  if (data.payload.op === 'u') {
    ...
    if (
      data.payload.after?.status === OpportunityMatchStatus.CandidateReview &&
      data?.payload.before?.status !== OpportunityMatchStatus.CandidateReview
    ) {
      await notifyOpportunityMatchCandidateReview({
        con,
        logger,
        data: data.payload.after!,
      });
...
const onPostAnalyticsChange = async (
  ...
  const clickIncrement = clicks - prevClicks;

  await checkQuestProgress({
    con,
    logger,
    userId: sharePost.authorId,
    eventType: QuestEventType.SharePostClick,
    incrementBy: clickIncrement,
  });
