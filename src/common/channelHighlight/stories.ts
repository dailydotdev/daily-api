...
// When the cron picks up a brand-new post that becomes a child of a collection,
// the post_relation row may be inserted *after* fetchRelations runs but before
// the run completes. The candidate is then sent to Bragi as a standalone story
// even though the collection (and possibly its sibling articles) was already
// covered by an earlier highlight. Re-check membership against fresh relations
// right before publishing and drop any admitted item whose collection — or any
// of that collection's other children — is already in the live or retired set.
export const dropAdmittedHighlightsAlreadyCovered = ({
  admittedHighlights,
  fallbackPostIds,
  ...
  admittedHighlights: HighlightItem[];
  fallbackPostIds: Map<string, string>;
  ...
  if (admittedHighlights.length === 0 || collectionByChild.size === 0) {
    return admittedHighlights;
  }
  ...
  const isCovered = (postId: string): boolean => {
    ...
    const sharedAlias = fallbackPostIds.get(postId);
    if (
      sharedAlias &&
      (currentHighlightPostIds.has(sharedAlias) ||
        retiredHighlightPostIds.has(sharedAlias))
    ) {
      return true;
    }
    return false;
  ...
  return admittedHighlights.filter((item) => {
    const underlying = shareToUnderlying.get(item.postId) ?? item.postId;
    const collectionId = collectionByChild.get(underlying);
    if (!collectionId) {
      return true;
    }
