...
export class PollVotes1757630994202 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
            ...
            BEGIN
                IF NEW."pollVoteOptionId" IS NOT NULL THEN
                    ...
                    WHERE id = (SELECT "postId" FROM poll_option WHERE id = NEW."pollVoteOptionId");
                END IF;
    ...
    await queryRunner.query(`
            ...
            RETURNS TRIGGER AS $$
            BEGIN
                IF OLD."pollVoteOptionId" IS DISTINCT FROM NEW."pollVoteOptionId" THEN
                    ...
                    ELSIF OLD."pollVoteOptionId" IS NOT NULL AND NEW."pollVoteOptionId" IS NULL THEN
                        ...
                        WHERE id = (SELECT "postId" FROM poll_option WHERE id = OLD."pollVoteOptionId");
                    
                    ELSIF OLD."pollVoteOptionId" IS NOT NULL AND NEW."pollVoteOptionId" IS NOT NULL THEN
                        UPDATE poll_option SET "numVotes" = GREATEST("numVotes" - 1, 0) WHERE id = OLD."pollVoteOptionId";
                        UPDATE poll_option SET "numVotes" = "numVotes" + 1 WHERE id = NEW."pollVoteOptionId";
                        
                        IF (SELECT "postId" FROM poll_option WHERE id = OLD."pollVoteOptionId") != 
                           (SELECT "postId" FROM poll_option WHERE id = NEW."pollVoteOptionId") THEN
                            UPDATE post SET "numPollVotes" = GREATEST("numPollVotes" - 1, 0) 
                            WHERE id = (SELECT "postId" FROM poll_option WHERE id = OLD."pollVoteOptionId");
                            UPDATE post SET "numPollVotes" = "numPollVotes" + 1 
                            WHERE id = (SELECT "postId" FROM poll_option WHERE id = NEW."pollVoteOptionId");
