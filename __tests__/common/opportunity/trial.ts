...
describe('Super Agent Trial helpers', () => {
  describe('getSuperAgentTrialConfig and isSuperAgentTrialEnabled', () => {
    it('should return default config when remoteConfig is not set', () => {
      const originalConfig = remoteConfig.vars.superAgentTrial;
      ...
      expect(isSuperAgentTrialEnabled()).toBe(false);

      remoteConfig.vars.superAgentTrial = originalConfig;
    });

    it('should return remoteConfig values when set', () => {
      const originalConfig = remoteConfig.vars.superAgentTrial;
      ...
      expect(config).toMatchObject({
        enabled: true,
        durationDays: 14,
        features: { batchSize: 200 },
      });
      expect(isSuperAgentTrialEnabled()).toBe(true);
