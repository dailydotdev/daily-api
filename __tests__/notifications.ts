...
describe('mutation readNotifications', () => {
  ...
  it('should set unread notifications as read v2', async () => {
    ...
    const user1 = await con.getRepository(User).findOneBy({ id: '1' });
    expect(user1?.unreadNotificationsCount).toBe(0);
  });
});

describe('unreadNotificationsCount trigger', () => {
  it('should increment counter when public unread notification is inserted', async () => {
    const notifs = await con.getRepository(NotificationV2).save([
      { ...notificationV2Fixture },
      {
        ...notificationV2Fixture,
  ...
  it('should not increment counter for private notifications', async () => {
    const notifs = await con
      .getRepository(NotificationV2)
      .save([{ ...notificationV2Fixture }]);
    await con.getRepository(UserNotification).insert([
  ...
  it('should not increment counter for already-read notifications', async () => {
    const notifs = await con
      .getRepository(NotificationV2)
      .save([{ ...notificationV2Fixture }]);
    await con.getRepository(UserNotification).insert([
  ...
  it('should decrement counter when notification is deleted', async () => {
    const notifs = await con
      .getRepository(NotificationV2)
      .save([{ ...notificationV2Fixture }]);
    await con.getRepository(UserNotification).insert([
    ...
    let user = await con.getRepository(User).findOneBy({ id: '1' });
    expect(user?.unreadNotificationsCount).toBe(1);

    await con.getRepository(UserNotification).delete({
      userId: '1',
      notificationId: notifs[0].id,
    });
    user = await con.getRepository(User).findOneBy({ id: '1' });
    expect(user?.unreadNotificationsCount).toBe(0);
  });
});
